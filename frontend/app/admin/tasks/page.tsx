...
export default function AdminTasksPage() {
  ...
  const filteredTasks = tasks.filter((task) => {
    ...
    return departmentMatch && userMatch && priorityMatch && statusMatch && assignedByMatch && searchMatch;
  });
  ...
  return (
    <div className="min-h-screen text-card-foreground p-8 font-sans rounded-xl relative">
      ...
      {showFilters && (
        <div className="mb-8 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 bg-card/60 backdrop-blur-md p-4 rounded-xl border border-border shadow-md">
          <div className="flex flex-col">
            <label className="block text-xs font-semibold text-muted-foreground mb-1">Department</label>
            <SelectField value={selectedDepartment} onValueChange={setSelectedDepartment}>
          ...
          <div className="flex flex-col">
            ...
            <SelectField value={selectedUser} onValueChange={setSelectedUser}>
              ...
              <SelectContent>
                <SelectItem value="All">All</SelectItem>
                {users.filter(u => u.role !== "MANAGER").map((u) => (<SelectItem key={u.id} value={u.name}>{u.name}</SelectItem>))}
              </SelectContent>
            </SelectField>
          </div>
          <div className="flex flex-col">
            <label className="block text-xs font-semibold text-muted-foreground mb-1">Assigned By</label>
            <SelectField value={selectedAssignedBy} onValueChange={setSelectedAssignedBy}>
              <SelectTrigger className="w-full bg-card border-border"><SelectValue placeholder="All" /></SelectTrigger>
              <SelectContent>
                <SelectItem value="All">All</SelectItem>
                {users.filter(u => u.role === "MANAGER" || u.role === "ADMIN").map((u) => (<SelectItem key={u.id} value={u.name}>{u.name}</SelectItem>))}
              </SelectContent>
            </SelectField>
          </div>
          <div className="flex flex-col">
